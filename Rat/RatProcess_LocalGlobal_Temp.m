function RatProcess_LocalGlobal_Temp(ROOTPATH, protStr, date)
%% Load data
baseline_Window = evalin('base', 'baseline_Window');
FigPATH = evalin('base', 'FigPATH');
for pIndex = 1 : length(protStr)
    ...
    try
        load(fullfile(FigPATH, protStr(pIndex), "Rat", date, "res.mat"), "trialAll", "chSpikeLfp");
    catch
        disp(string(date));
        break;
    end
