...
for tIdx = 1 : length(trialOnset)
    ...
    if BaseICI < InsertICI
        trialType = cellfun(@(InNum) strcat(string(BaseICI), '_', string(InsertICI), 'ms_IncN', string(InNum)), ...
    ...
    elseif BaseICI > InsertICI
        trialType = cellfun(@(InNum) strcat(string(BaseICI), '_', string(InsertICI), 'ms_DecN', string(InNum)), ...
                            mat2cell(Insert_Num, ones([size(Insert_Num, 1), 1])));
        for i = 1:length(Insert_Num)
